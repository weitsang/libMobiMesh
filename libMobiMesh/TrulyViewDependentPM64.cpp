...
TrulyViewDependentPM64::TrulyViewDependentPM64() { }

TrulyViewDependentPM64::TrulyViewDependentPM64(const std::string& filename)
...
void TrulyViewDependentPM64::get_active_cuts_fast (VPIter vpit,
                                                   ...
                                                   int &v1vl, int &vrv1)
{
    ...
    // the assertion will be not always true, e.g. in xyzrgb_dragon.vpm, thus not essential.
    // assert (vl != -1 && vr != -1);
}

TrulyViewDependentPM64::ZsData TrulyViewDependentPM64::loadVs (unsigned int pos)
{
    if (mmapped_)
...
void TrulyViewDependentPM64::force_vsplit (VPIter vpit)
{
    const ZsData& zs = loadVs(vpit->pos);

    unsigned int fund_lcut_tree = zs.fund_ccut_tree >> 16;
    unsigned int fund_rcut_tree = zs.fund_ccut_tree << 16 >> 16;
    unsigned int fund_lcut_node = zs.fund_lcut_node;
    unsigned int fund_rcut_node = zs.fund_rcut_node;
...
inline void TrulyViewDependentPM64::splt_index (int v0, int v1)
{
...
inline void TrulyViewDependentPM64::ecol_index (int v0, int v1) 
{
...
void TrulyViewDependentPM64::readVDPM(const char* _filename)
{
    char                        fileformat[16];
    ...
    s_details_       = sizeof(OpenMesh::Vec3f)+sizeof(OpenMesh::Vec3f)+sizeof(OpenMesh::Vec3f)+sizeof(unsigned int)+sizeof(unsigned int)+sizeof(unsigned int)+sizeof(int); 
